...
This version now supports two modes:
  • Normal Mode (default): ROI types are Cell, Parent Dendrite, and Dendritic Spine.
  • Dendrites/Axons Mode: ROI types are Parent Dendrite, Dendritic Spine, Parent Axon, and Axonal Bouton.
     In this mode, file names change to “ROIs_dendrite_axon_mode.npy” and
     “ROIs_dendrite_axon_mode_conversion.npy”, and the roi-type list is:
         [roi type, parent dendrite ID, dendritic spine ID, parent axon ID, axonal bouton ID].

A toggle button in the top–right allows switching modes.
When loading a Suite2p folder, the file names in the SpinesGUI folder determine which mode to use.

WARNING: In Dendrites/Axons Mode the hierarchy is enforced:
  - A dendritic spine must be associated with a parent dendrite.
  - An axonal bouton must be associated with a parent axon.
  - There is no cell ROI in this mode.

Requirements:
  - PyQt5, numpy, matplotlib, suite2p (with appropriate submodules)
"""
...
# --- New Helper Methods for Dendrites/Axons Mode ---
# These methods compute the next ID for each ROI type in dendrites/axons mode.
class ModeHelpers:
    @staticmethod
...
class ROIItem(QGraphicsPolygonItem):
    def __init__(self, roi_id, polygon, roi_info, main_window, parent=None):
        ...
        self.roi_id = roi_id
        self.roi_info = roi_info  # For normal mode: [roi type, cellID, parent dendrite ID, dendritic spine ID]
                                  # For dendrites/axons mode: [roi type, parent dendrite ID, dendritic spine ID, parent axon ID, axonal bouton ID]
        self.main_window = main_window
        # Set pen using semi-transparent colors.
        # For Normal mode, use the old color scheme.
        if self.main_window.mode == "normal":
            base_color = {0: Qt.blue, 1: Qt.red, 2: Qt.green}.get(self.roi_info["roi-type"][0], Qt.gray)
        else:
            # For dendrites/axons mode, assign:
            # 0: Parent Dendrite (blue), 1: Dendritic Spine (red), 2: Parent Axon (green), 3: Axonal Bouton (magenta)
    ...
    def update_vertex_markers(self):
        ...
        self.vertex_markers = []
        r = 1  # Vertex marker radius set to 1.
    ...
    def delete_roi(self):
        ...
        if self.main_window.mode == "normal":
            ...
            # Dendrites/Axons mode deletion.
            # For dendritic spine (type 1), delete if associated with same parent dendrite.
            # For axonal bouton (type 3), delete if associated with same parent axon.
            if typ == 1:
...
class CustomGraphicsView(QGraphicsView):
    ...
    def mouseMoveEvent(self, event):
        ...
        if (self.parent_window.image_width is not None and self.parent_window.image_height is not None and
            0<= scene_pos.x() <= self.parent_window.image_width and 0<= scene_pos.y() <= self.parent_window.image_height):
...
class ROITableWindow(QDialog):
    ...
    def init_ui(self):
        ...
        if self.main_window.mode == "dendrites_axons":
            columns = ["ROI #", "ROI Type", "Parent Dendrite ID", "Dendritic Spine ID",
                       "Parent Axon ID", "Axonal Bouton ID", "Plane", "ROI Coordinates"]
        else:
            columns = ["ROI #", "ROI Type", "Cell ID", "Parent Dendrite ID",
    ...
    def populate_table(self):
        ...
        for row, roi_id in enumerate(sorted(self.roi_data.keys())):
            ...
            if self.main_window.mode == "dendrites_axons":
                # Expect roi-type list: [roi type, parent dendrite ID, dendritic spine ID, parent axon ID, axonal bouton ID]
                ...
                # Normal mode: roi-type list: [roi type, cell ID, parent dendrite ID, dendritic spine ID]
...
class ConversionTableDialog(QDialog):
    ...
    def init_ui(self):
        layout = QVBoxLayout()
        self.table = QTableWidget()
        # In Conversion table, we add an extra column for "Conversion Index"
        if self.parent().mode == "dendrites_axons":
            columns = ["ROI #", "ROI Type", "Parent Dendrite ID", "Dendritic Spine ID",
                       "Parent Axon ID", "Axonal Bouton ID", "Plane", "ROI Coordinates", "Conversion", "Conversion Index"]
        else:
            columns = ["ROI #", "ROI Type", "Cell ID", "Parent Dendrite ID",
...
class ROITypeDialog(QDialog):
    ...
    def update_association(self, existing_cells, existing_parents):
        ...
        # In normal mode, associations are required for Parent Dendrite (with a Cell) and Dendritic Spine.
        # In dendrites/axons mode, only dendritic spine and axonal bouton require association.
        if self.parent().mode == "normal":
            if roi_type == 1:
            ...
            if roi_type == 1:  # Dendritic Spine requires Parent Dendrite association.
                # existing_cells not used in this mode; instead use available Parent Dendrite ROIs.
                available = [roi_id for roi_id, info in self.parent().roi_data.items() if info["plane"] == self.parent().plane_order[self.parent().current_plane_index] and info["roi-type"][0] == 0]
                if not available:
                    QMessageBox.warning(self, "No Parent Dendrites", "No Parent Dendrite ROIs available.")
                    self.reject()
                    return
                ...
                self.highlight_association()
            elif roi_type == 3:  # Axonal Bouton requires Parent Axon association.
                available = [roi_id for roi_id, info in self.parent().roi_data.items() if info["plane"] == self.parent().plane_order[self.parent().current_plane_index] and info["roi-type"][0] == 2]
                if not available:
    ...
    def on_ok(self):
        ...
        if self.shape=="polygon":
            try:
                ...
                QMessageBox.warning(self, "Invalid Input", "Enter an integer between 3 and 10 for polygon sides.")
                return
        self.accept()
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.mode = "normal"  # Default mode; possible values: "normal" or "dendrites_axons"
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # Toggle button for mode switching, placed at top-right.
        ...
        # Top layout: add buttons, with mode toggle at the right.
    ...
    def start_roi_addition(self):
        """Initiate the process of adding a new ROI."""
        if not self.plane_order:
            QMessageBox.warning(self, "Error", "No plane loaded.")
            return

        # In normal mode, we might have cell ROIs and parent dendrite ROIs.
        # In dendrites/axons mode, the associations differ.
        if self.mode == "normal":
            ...
            # For dendrites/axons mode, associations use parent dendrites for dendritic spines,
            # and parent axons for axonal boutons.
            existing_cells = []  # Not used in dendrites/axons mode.
            existing_parents = [roi_id for roi_id, info in self.roi_data.items() 
                                if info["plane"] == self.plane_order[self.current_plane_index] 
                                and info["roi-type"][0] == 0]  # Parent Dendrite in dendrites/axons mode.

        # Open the ROI type selection dialog.
        ...
        # Open the ROI shape selection dialog.
        ...
        # If tracing is chosen, clear previous tracing data.
    ...
    def toggle_mode(self):
        # When toggled, ask for confirmation.
        ...
        # When mode is switched, reload the ROI files (if any) from SpinesGUI folder.
        self.load_existing_rois_mode()

    def load_existing_rois_mode(self):
        # Check the SpinesGUI folder for ROI file names.
        ...
        roi_files = [f for f in os.listdir(spines_gui_folder) if f.endswith(".npy")]
        # If there are no ROI files, do not auto-detect mode (allow user to choose)
        if not roi_files:
            return
        mode_detected = "normal"
        for f in roi_files:
            if "dendrite_axon_mode" in f:
                mode_detected = "dendrites_axons"
                break
        if mode_detected == "dendrites_axons":
            QMessageBox.information(self, "Mode Detected", "Dendrites/Axons mode was detected.")
            self.mode = "dendrites_axons"
            self.mode_toggle.setChecked(True)
            self.mode_toggle.setText("Dendrites/Axons Mode")
        else:
            QMessageBox.information(self, "Mode Detected", "Normal mode was detected.")
            self.mode = "normal"
            self.mode_toggle.setChecked(False)
            self.mode_toggle.setText("Normal Mode")
    ...
    def update_contrast(self):
        ...
        try:
            if self.image_pixmap_item is None or self.image_pixmap_item.scene() is None:
                raise RuntimeError("Pixmap invalid")
            else:
    ...
    def load_suite2p_folder(self):
        ...
        if self.root_folder is not None:
            reply = QMessageBox.question(self, "Change Folder", 
        ...
        # Check for existing ROI file names to auto-detect mode.
        self.load_existing_rois_mode()
        # Load ROIs file based on mode.
        ...
        if os.path.exists(rois_file):
            try:
                ...
                print(f"[DEBUG] Error loading ROIs file: {e}")
    ...
    def update_plane_display(self):
        ...
        self.roi_items.clear()  # Clear ROI items from previous plane.
    ...
    # For Normal mode, _create_roi_from_points uses the previous structure.
    # For dendrites/axons mode, it uses the new roi-type list.
    def _create_roi_from_points(self, pts):
        ...
        if self.mode == "normal":
            ...
            # Dendrites/Axons mode: ROI type list: [roi type, parent dendrite ID, dendritic spine ID, parent axon ID, axonal bouton ID]
            if self.pending_roi_type[0] == 0:  # Parent Dendrite
                pd_id = ModeHelpers.get_next_parent_dendrite_id(self.roi_data)
                roi_type_list = [0, pd_id, 0, 0, 0]
            elif self.pending_roi_type[0] == 1:  # Dendritic Spine
                associated_parent_key = self.pending_roi_type[1]
                parent_roi = self.roi_data.get(associated_parent_key)
                ...
                ds_id = ModeHelpers.get_next_dendritic_spine_id(self.roi_data, pd_id)
                roi_type_list = [1, pd_id, ds_id, 0, 0]
            elif self.pending_roi_type[0] == 2:  # Parent Axon
                pa_id = ModeHelpers.get_next_parent_axon_id(self.roi_data)
                roi_type_list = [2, 0, 0, pa_id, 0]
            elif self.pending_roi_type[0] == 3:  # Axonal Bouton
                associated_parent_key = self.pending_roi_type[1]
                parent_roi = self.roi_data.get(associated_parent_key)
                ...
                ab_id = ModeHelpers.get_next_axonal_bouton_id(self.roi_data, pa_id)
    ...
    # In dendrites/axons mode, use the ModeHelpers functions.
    def get_next_parent_dendrite_id(self):
        return ModeHelpers.get_next_parent_dendrite_id(self.roi_data)
    def get_next_dendritic_spine_id(self, parent_dendrite_id):
        return ModeHelpers.get_next_dendritic_spine_id(self.roi_data, parent_dendrite_id)
    ...
    def extract_rois(self):
        ...
        for plane in self.plane_data.keys():
            ...
            try:
                ...
                print(f"[DEBUG] Saved stat1 for plane {plane} in {plane_folder}")
            except Exception as e:

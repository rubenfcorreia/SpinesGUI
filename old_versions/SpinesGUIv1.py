...
Revised to ensure that associated ROI information is refreshed and stored correctly.
Key changes:
  • When the user clicks “OK” in the Select ROI Type dialog, the current selection is saved.
  • For Parent dendrite ROIs (type 1), the associated Cell ROI’s cell ID is taken from the stored ROIs.
  • For Dendritic spine ROIs (type 2), the associated Parent dendrite ROI is required and its cell ID and parent dendrite ID are used.
Extra debug prints have been added.
"""
...
class ROIItem(QGraphicsPolygonItem):
    ...
    def update_vertex_markers(self):
        ...
        for pt in self.polygon():
            rect = QRectF(pt.x()-radius, pt.y()-radius, radius*2, radius*2)
...
class ROITypeDialog(QDialog):
    ...
    def accept(self):
        # Save the current selections into private variables.
        try:
...
class ROIShapeDialog(QDialog):
    ...
    def init_ui(self):
        ...
        self.sides_input.setEnabled(False)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Suite2p ROI Selector")
        self.setGeometry(50, 50, 1200, 800)
        self.showFullScreen()

        self.root_folder = None
        self.plane_data = OrderedDict()  # keys: plane number; values: dict with keys: meanImg, yrange, xrange, folder
        self.plane_order = []
        ...
        self.pending_roi_shape = None  # "rectangle" or "polygon"
    ...
    def load_suite2p_folder(self):
        ...
        if os.path.exists(rois_file):
            try:
                ...
                print("[DEBUG] Error loading ROIs.npy: %s" % str(e))
    ...
    def update_plane_display(self):
        ...
        rect_item = self.scene.addRect(valid_rect, QPen(Qt.yellow, 2))
    ...
    def start_roi_addition(self):
        ...
        QMessageBox.information(self, "ROI Drawing",
                                "Click two points within the valid ROI area.\nFor rectangle, the clicks define opposite corners.\nFor polygon, a regular polygon will be inscribed.")
    ...
    def remove_roi(self, roi_id):
        if roi_id in self.roi_items:
            self.scene.removeItem(self.roi_items[roi_id])
